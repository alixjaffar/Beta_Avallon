...
export const WebsiteEditor: React.FC<WebsiteEditorProps> = ({ site, onUpdate, onClose }) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorMessage.includes('VERCEL_TOKEN') || errorMessage.includes('not configured')) {
          errorMessage = 'Vercel is not configured. Please add VERCEL_TOKEN to your Render environment variables.';
          ...
          errorMessage = 'GitHub connection issue (optional). Trying Vercel-only deployment...';
        }
      ...
      // Also update local state immediately so Domain button enables right away
      if (result.vercelProjectId) {
        setLocalVercelProjectId(result.vercelProjectId);
      }
      ...
      if (error.name === 'AbortError') {
        errorMessage = "Deployment timed out. Please try again or use Download for self-hosting.";
      }
  ...
  // Handle injecting widget/code into all existing pages
  const handleInjectToAllPages = async (code: { html: string; css: string; js: string }) => {
    ...
    if (existingPages.length === 0) {
      toast({
        ...
        description: "Import a website first, then add widgets.",
        variant: "destructive",
      });
      return;
    }
    ...
    for (const pageName of existingPages) {
      ...
      if (code.css) {
        ...
        if (html.includes('</head>')) {
          ...
          html = html.replace('<body', `<head>${styleTag}</head>\n<body`);
        }
      }
    ...
    try {
      ...
      toast({
        title: "Widget Added!",
        description: `Successfully added to ${injectedCount} page${injectedCount > 1 ? 's' : ''}. Publish to see changes.`,
      });
    } catch (error) {
      console.error('Error saving:', error);
      toast({
        title: "Saved Locally",
        description: "Widget added. Click Publish to deploy.",
      });
    }
  ...
  // Track vercelProjectId locally (so Domain button enables immediately after publish)
  const [localVercelProjectId, setLocalVercelProjectId] = useState(site.vercelProjectId || '');
  const [dnsRecords, setDnsRecords] = useState<Array<{ type: string; name: string; value: string }>>([]);
  
  // Custom Scripts/Widgets state
  const [showScriptsModal, setShowScriptsModal] = useState(false);
  const [customScriptCode, setCustomScriptCode] = useState('');
  const [scriptApplyTo, setScriptApplyTo] = useState<'all' | 'current'>('all');
  const [savedScripts, setSavedScripts] = useState<Array<{ id: string; code: string; applyTo: 'all' | string; name: string }>>([]);
  
  // WordPress Full-Width Fix state
  const [wpFullWidthFix, setWpFullWidthFix] = useState(false);
  
  // Load saved scripts from websiteContent
  useEffect(() => {
    if (currentWebsiteContent?._customScripts) {
      setSavedScripts(currentWebsiteContent._customScripts);
    }
  }, [currentWebsiteContent?._customScripts]);
  
  // Load WordPress full-width fix setting
  useEffect(() => {
    if (currentWebsiteContent?._wpFullWidthFix !== undefined) {
      setWpFullWidthFix(currentWebsiteContent._wpFullWidthFix);
    }
  }, [currentWebsiteContent?._wpFullWidthFix]);
  
  // Save custom script
  const handleSaveScript = async () => {
    if (!customScriptCode.trim()) return;
    
    const scriptName = customScriptCode.includes('intercom') ? 'Intercom Widget' :
                       customScriptCode.includes('drift') ? 'Drift Chat' :
                       customScriptCode.includes('crisp') ? 'Crisp Chat' :
                       customScriptCode.includes('tidio') ? 'Tidio Chat' :
                       customScriptCode.includes('hubspot') ? 'HubSpot Chat' :
                       customScriptCode.includes('zendesk') ? 'Zendesk Widget' :
                       'Custom Widget';
    
    const newScript = {
      id: `script_${Date.now()}`,
      code: customScriptCode.trim(),
      applyTo: scriptApplyTo === 'all' ? 'all' : effectiveCurrentPage,
      name: scriptName,
    };
    ...
    try {
      ...
      toast({
        title: "Widget Saved!",
        description: `${scriptName} will be added to ${scriptApplyTo === 'all' ? 'all pages' : effectiveCurrentPage} on next publish.`,
      });
      setCustomScriptCode('');
  ...
  // Toggle WordPress full-width fix
  const handleToggleWpFix = async () => {
    const newValue = !wpFullWidthFix;
    setWpFullWidthFix(newValue);
    
    const updatedContent = {
      ...currentWebsiteContent,
      _wpFullWidthFix: newValue,
    };
    setCurrentWebsiteContent(updatedContent);
    ...
    try {
      ...
      toast({ 
        title: newValue ? "Layout Fix Enabled" : "Layout Fix Disabled",
        description: newValue 
          ? "Full-width sections will span edge-to-edge. Re-publish to apply." 
          : "Layout fix disabled. Re-publish to apply.",
      });
    } catch (error) {
      console.error('Failed to toggle WP fix:', error);
      toast({
        title: "Error",
    ...
    try {
      ...
      const fetchPromises = Array.from(imageUrls).map(async (url) => {
        try {
          ...
          // Skip if the response is HTML (error page) instead of an image
          if (blob.type.includes('text/html') || blob.type.includes('application/json')) {
            console.warn('Skipping non-image response:', url, blob.type);
            return;
          }
          ...
          // Continue without this image - don't block the download
        }
      });
...
`);
      ...
      const successImages = Object.keys(imageMapping).length;
      
      toast({
        title: "Downloaded!",
        description: successImages > 0 
          ? `Your website has been downloaded with ${successImages}/${totalImages} images. Extract and host anywhere!`
          : `Your website has been downloaded. Some images may need to be downloaded manually.`,
      });
    } catch (error: any) {
    ...
    <div className={`font-display overflow-hidden h-screen flex flex-col antialiased ${isLight ? 'bg-gray-50 text-slate-900' : 'bg-background-dark text-white'}`}>
      ...
      <header className={`flex flex-col sm:flex-row items-start sm:items-center justify-between border-b px-3 sm:px-6 py-2 sm:py-3 min-h-16 shrink-0 z-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#111218] border-panel-border'}`}>
        ...
        <div className="flex items-center gap-1 sm:gap-3 w-full sm:w-auto justify-end flex-wrap">
          ...
          {/* WordPress Layout Fix Button */}
          <button 
            onClick={handleToggleWpFix}
            className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
              wpFullWidthFix 
            ...
            title={wpFullWidthFix ? "Layout fix enabled - click to disable" : "Fix full-width sections (for imported WordPress sites)"}
          >
            <span className="material-symbols-outlined text-[16px] sm:text-[18px]">
              {wpFullWidthFix ? 'check_circle' : 'fit_screen'}
            </span>
            <span className="hidden sm:inline">{wpFullWidthFix ? 'Fix On' : 'Fix Layout'}</span>
          </button>
          
          {/* Widgets/Scripts Button */}
          <button 
            onClick={() => setShowScriptsModal(true)}
            className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${isLight ? 'bg-slate-200 hover:bg-slate-300 text-slate-700' : 'bg-amber-600 hover:bg-amber-500 text-white'}`}
            title="Add chat widgets, analytics, or custom scripts"
          >
            <span className="material-symbols-outlined text-[16px] sm:text-[18px]">widgets</span>
            <span className="hidden sm:inline">Widgets</span>
            {savedScripts.length > 0 && (
              <span className={`ml-1 px-1.5 py-0.5 text-[10px] rounded-full ${isLight ? 'bg-amber-500 text-white' : 'bg-amber-400 text-amber-900'}`}>
                {savedScripts.length}
              </span>
            )}
          </button>
                  ...
                  <div className="p-4 border-t border-panel-border bg-surface-dark/50">
                    <div className="flex items-center justify-between gap-2 mb-2">
                      ...
                      <button
                        onClick={() => {
                          if (selectedElement) {
                            const elementRef = `[${selectedElement.tagName}${selectedElement.id ? `#${selectedElement.id}` : ''}${selectedElement.className ? `.${selectedElement.className.split(' ')[0]}` : ''}: "${selectedElement.content?.substring(0, 50) || ''}${selectedElement.content && selectedElement.content.length > 50 ? '...' : ''}"]`;
                            setAiAssistInput(prev => prev ? `${prev} → ${elementRef}` : `Change this element ${elementRef}: `);
                            toast({
                              title: "Element Referenced!",
                              description: "Now describe what you want to change.",
                            });
                          }
                        }}
                        className="flex items-center gap-1 px-2 py-1 rounded-md bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 text-amber-400 text-[10px] font-medium transition-colors"
                        title="Reference the selected element in your prompt"
                      >
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        placeholder={selectedElement ? `Describe change for ${selectedElement.tagName}...` : "Select an element, then describe change..."}
                        className="w-full px-3 py-2 pr-10 bg-background-dark border border-panel-border rounded-lg text-sm text-white placeholder-gray-500 resize-none h-20"
                      />
                      <button
                        onClick={handleAiAssist}
                        disabled={!aiAssistInput.trim() || isAiAssistLoading}
                        className="absolute right-2 bottom-2 text-primary hover:text-primary/80 disabled:opacity-50"
                      >
                        {isAiAssistLoading ? (
                    ...
                    {selectedElement && (
                      <p className="text-[10px] text-gray-500 mt-1.5">
                        💡 Tip: Click "Reference This" to target the selected <span className="text-amber-400">{selectedElement.tagName}</span> element
                      </p>
                    )}
                  </div>
                </div>
      ...
      <ImportWebsiteModal
        ...
        onInjectToAllPages={handleInjectToAllPages}
        existingPages={Object.keys(currentWebsiteContent).filter(k => k.endsWith('.html') && !k.startsWith('_'))}
        isLight={isLight}
      />
      ...
      {/* Custom Scripts/Widgets Modal */}
      {showScriptsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-sm">
          <div className={`w-full max-w-2xl max-h-[90vh] sm:max-h-[85vh] rounded-xl sm:rounded-2xl shadow-2xl overflow-y-auto ${isLight ? 'bg-white' : 'bg-[#1a1f2e] border border-panel-border'}`}>
            {/* Header */}
            <div className={`sticky top-0 z-10 flex items-center justify-between p-4 border-b ${isLight ? 'bg-white border-slate-200' : 'bg-[#1a1f2e] border-panel-border'}`}>
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg ${isLight ? 'bg-amber-100' : 'bg-amber-500/20'}`}>
                  <span className={`material-symbols-outlined ${isLight ? 'text-amber-600' : 'text-amber-400'}`}>widgets</span>
                </div>
                <h2 className={`text-lg font-bold ${isLight ? 'text-slate-800' : 'text-white'}`}>Widgets & Scripts</h2>
              </div>
              <button 
                onClick={() => { setShowScriptsModal(false); setCustomScriptCode(''); }}
                className={`size-8 flex items-center justify-center rounded-lg transition-colors ${isLight ? 'hover:bg-slate-100 text-slate-500' : 'hover:bg-panel-border text-gray-400 hover:text-white'}`}
              >
                <span className="material-symbols-outlined">close</span>
              </button>
            ...
            <div className="p-4 sm:p-6 space-y-6">
              ...
              {savedScripts.length > 0 && (
                <div>
                  <h3 className={`text-sm font-semibold mb-3 ${isLight ? 'text-slate-700' : 'text-gray-300'}`}>
                    Active Widgets ({savedScripts.length})
                  </h3>
                  <div className="space-y-2">
                    {savedScripts.map((script) => (
                      <div 
                        key={script.id} 
                        className={`flex items-center justify-between p-3 rounded-lg ${isLight ? 'bg-slate-100' : 'bg-surface-dark'}`}
                      >
                        <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-colors ${isLight ? 'hover:bg-red-100 text-red-500' : 'hover:bg-red-900/30 text-red-400'}`}
                          title="Remove widget"
                        >
              ...
              <div>
                ...
                {/* Apply To Selection */}
                <div className="mb-4">
                  <label className={`block text-xs font-medium mb-2 ${isLight ? 'text-slate-600' : 'text-gray-400'}`}>
                    Apply to:
                  </label>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setScriptApplyTo('all')}
                      className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        scriptApplyTo === 'all'
                          ? (isLight ? 'bg-amber-500 text-white' : 'bg-amber-500 text-white')
                          : (isLight ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-surface-dark text-gray-400 hover:bg-panel-border')
                      }`}
                    >
                    ...
                    <button
                      onClick={() => setScriptApplyTo('current')}
                      className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        scriptApplyTo === 'current'
                          ? (isLight ? 'bg-amber-500 text-white' : 'bg-amber-500 text-white')
                          : (isLight ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-surface-dark text-gray-400 hover:bg-panel-border')
                      }`}
                    >
                      <span className="material-symbols-outlined text-sm mr-1 align-middle">article</span>
                ...
                <div className="mb-4">
                  <label className={`block text-xs font-medium mb-2 ${isLight ? 'text-slate-600' : 'text-gray-400'}`}>
                    Paste your widget code (script tag):
                  </label>
                  <textarea
                    value={customScriptCode}
                    onChange={(e) => setCustomScriptCode(e.target.value)}
                    placeholder={'<script>\n  // Paste your chat widget, analytics, or any script here\n  // Example: Intercom, Drift, Crisp, Google Analytics, etc.\n</script>'}
                    className={`w-full h-40 p-3 rounded-lg font-mono text-xs resize-none ${
                      isLight 
                        ...
                        : 'bg-surface-dark border border-panel-border text-white placeholder-gray-500'
                ...
                <button
                  ...
                  disabled={!customScriptCode.trim()}
                ...
                >
                  <span className="material-symbols-outlined text-lg mr-2 align-middle">add_circle</span>
                  Add Widget
                </button>
              </div>
              
              {/* Help Text */}
              <div className={`p-4 rounded-lg text-xs ${isLight ? 'bg-blue-50 text-blue-700' : 'bg-blue-900/20 text-blue-300'}`}>
                <p className="font-medium mb-2">💡 Supported Widgets:</p>
                <ul className="list-disc list-inside space-y-1">
                  <li>Chat widgets (Intercom, Drift, Crisp, Tidio, HubSpot)</li>
                  <li>Analytics (Google Analytics, Hotjar, Mixpanel)</li>
                  <li>Any custom JavaScript or tracking code</li>
                </ul>
                <p className="mt-2 text-xs opacity-75">
                  Widgets are injected into your pages when you publish.
                </p>
              </div>
            </div>
          </div>
