...
(function() {
  ...
  window.addEventListener('message', function(e) {
    ...
    if (e.data.type === 'getHTML') {
      ...
      window.parent.postMessage({ type: 'htmlContent', data: { html: html, page: pageForSave, saveSeq: saveSeq } }, '*');
...
export const WebsiteEditor: React.FC<WebsiteEditorProps> = ({ site, onUpdate, onClose }) => {
  ...
  /** Bumps on each Visual Save — drop stale htmlContent if user saved again while getHTML was in flight. */
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'htmlContent') {
        const incomingSeq = event.data.data?.saveSeq as number | undefined;
        if (incomingSeq !== undefined && incomingSeq !== visualSaveSeqRef.current) {
          return;
        }
  ...
  const saveVisualChanges = async () => {
    ...
    visualSaveSeqRef.current += 1;
    const saveSeq = visualSaveSeqRef.current;
    // Request the current HTML from the iframe; include page key so async response applies to the right file
    iframeRef.current.contentWindow.postMessage({ type: 'getHTML', page: effectiveCurrentPage, saveSeq }, '*');
  ...
  const persistChangesToBackend = async (updatedContent: Record<string, string>) => {
    ...
    if (putBody.length > 4_000_000) {
      toast({
        title: "Save may be too large",
        description:
          "Several large embedded images make the request huge. Use hosted image URLs (not big uploads) or save fewer changes at once.",
        variant: "destructive",
      });
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 413) {
          toast({
            title: "Save rejected (too large)",
            description:
              "The server cannot accept this payload. Replace large uploads with hosted image URLs (or upload fewer images per save).",
            variant: "destructive",
          });
  ...
  // Custom Scripts/Widgets state (for managing imported widgets)
...
`);
  ...
  const dataUrlToBlob = (dataUrl: string): Blob | null => {
    try {
      ...
      // Strip whitespace from base64 (WordPress / HTML sometimes wraps long lines)
      const data = isBase64 ? match[3].replace(/\s+/g, '') : match[3];
   ...
   *
   * NOTE: Never use RegExp.replace() for data URLs — base64 strings can be MB+ and exceed JS
   * regex size limits ("Invalid regular expression: regular expression too large"). Use split/join.
   */
  const uploadDataUrlsInHtml = async (html: string): Promise<string> => {
    const dataUrlPattern = /\bdata:image\/[a-zA-Z0-9+.=-]+;base64,[A-Za-z0-9+/=\s]+/g;
    const matches = html.match(dataUrlPattern);
    if (!matches || matches.length === 0) return html;

    // Dedupe exact substrings as they appear in HTML (required for split/replace)
    const uniqueDataUrls = [...new Set(matches)];
    console.log(`[Save] Found ${uniqueDataUrls.length} embedded data URLs to upload`);

    const mapping: Record<string, string> = {};

    /** Upload in small parallel batches — faster than sequential, avoids hammering the API */
    const CONCURRENCY = 4;
    for (let i = 0; i < uniqueDataUrls.length; i += CONCURRENCY) {
      const chunk = uniqueDataUrls.slice(i, i + CONCURRENCY);
      await Promise.all(
        chunk.map(async (dataUrl) => {
          const blob = dataUrlToBlob(dataUrl);
          if (!blob) return;
          const serverUrl = await uploadBlobToServer(blob, false);
          if (serverUrl) mapping[dataUrl] = serverUrl;
        })
      );
    }

    const uploaded = Object.keys(mapping).length;
    console.log(`[Save] Uploaded ${uploaded}/${uniqueDataUrls.length} images`);

    let result = html;
    for (const [dataUrl, serverUrl] of Object.entries(mapping)) {
      // Literal string replace — safe for multi-MB strings (no RegExp size limit)
      const parts = result.split(dataUrl);
      if (parts.length > 1) {
        result = parts.join(serverUrl);
      }
    }
    return result;
  };
                    ...
                    {propertiesTab === 'style' && (
                      <>
                        ...
                        <div className="space-y-3 pt-3 border-t border-panel-border">
                          ...
                          <div className="grid grid-cols-4 gap-1">
                            <button
                              onClick={() => updateElementStyle('clipPath', 'none')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Remove mask"
                            >
                            ...
                            <button
                              onClick={() => updateElementStyle('clipPath', 'polygon(20% 0, 100% 0, 100% 100%, 0 100%)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Diagonal cut from top-left"
                            >
                              <svg className="w-6 h-4" viewBox="0 0 24 16">
                                <polygon points="5,0 24,0 24,16 0,16" fill="currentColor" opacity="0.5" stroke="currentColor" strokeWidth="1"/>
                              </svg>
                              Diag L
                            </button>
                            <button
                              onClick={() => updateElementStyle('clipPath', 'polygon(0 0, 80% 0, 100% 100%, 0 100%)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Diagonal cut from top-right"
                            >
                              <svg className="w-6 h-4" viewBox="0 0 24 16">
                                <polygon points="0,0 19,0 24,16 0,16" fill="currentColor" opacity="0.5" stroke="currentColor" strokeWidth="1"/>
                              </svg>
                              Diag R
                            </button>
                            <button
                              onClick={() => updateElementStyle('clipPath', 'circle(50% at 50% 50%)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Circular crop"
                            >
                              <div className="w-5 h-5 border border-current rounded-full" style={{background: 'currentColor', opacity: 0.5}} />
                              Circle
                            </button>
                            <button
                              onClick={() => updateElementStyle('clipPath', 'polygon(8% 0, 100% 0, 92% 100%, 0 100%)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Parallelogram slant left"
                            >
                              <svg className="w-6 h-4" viewBox="0 0 24 16">
                                <polygon points="2,0 24,0 22,16 0,16" fill="currentColor" opacity="0.5" stroke="currentColor" strokeWidth="1"/>
                              </svg>
                              Slant L
                            </button>
                            <button
                              onClick={() => updateElementStyle('clipPath', 'polygon(0 0, 92% 0, 100% 100%, 8% 100%)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Parallelogram slant right"
                            >
                              <svg className="w-6 h-4" viewBox="0 0 24 16">
                                <polygon points="0,0 22,0 24,16 2,16" fill="currentColor" opacity="0.5" stroke="currentColor" strokeWidth="1"/>
                              </svg>
                              Slant R
                            </button>
                            <button
                              onClick={() => updateElementStyle('clipPath', 'inset(0 0 0 0 round 20px)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Rounded rectangle"
                            >
                              <div className="w-6 h-4 border border-current rounded-lg" style={{background: 'currentColor', opacity: 0.5}} />
                              Round
                            </button>
                            <button
                              onClick={() => updateElementStyle('clipPath', 'ellipse(50% 40% at 50% 50%)')}
                              className={`flex flex-col items-center gap-0.5 p-2 rounded border text-[9px] transition-colors ${isLight ? 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600' : 'bg-surface-dark hover:bg-panel-border border-panel-border text-gray-400'}`}
                              title="Ellipse/oval shape"
                            >
                              <div className="w-6 h-4 border border-current rounded-full" style={{background: 'currentColor', opacity: 0.5}} />
                              Oval
                            </button>
                          </div>
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] text-gray-500">Custom clip-path (advanced)</label>
                            <input
                              type="text"
                              placeholder="polygon(0 0, 100% 0, 100% 100%, 0 100%)"
                              className="w-full px-2 py-1.5 bg-surface-dark border border-panel-border rounded text-xs text-white placeholder-gray-600 font-mono"
                              onBlur={(e) => {
                                if (e.target.value.trim()) {
                                  updateElementStyle('clipPath', e.target.value.trim());
                                }
                              }}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                        ...
                        {isImageElement(selectedElement) && (
                          <div className="space-y-3 mb-4">
                            ...
                            <p className="text-[10px] text-gray-500 leading-snug">
                              You can replace several images on this page, then press <strong className="text-gray-400">Save</strong> once — all edits are captured together.
                            </p>
                            <div className="space-y-3">
                  ...
                  <div className={`px-6 py-4 border-b ${isLight ? 'border-slate-200' : 'border-panel-border'}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      >
                        ...
                        <span className="text-[10px]">Slider</span>
                      </button>
                    </div>
                  </div>
