...
`);
  ...
  return (
    <div className={`font-display overflow-hidden h-screen flex flex-col antialiased ${isLight ? 'bg-gray-50 text-slate-900' : 'bg-background-dark text-white'}`}>
      {/* Top Navigation Bar */}
      <header className={`flex items-center justify-between border-b px-6 py-3 h-16 shrink-0 z-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#111218] border-panel-border'}`}>
        {/* Logo */}
        <div className="flex items-center gap-3">
          <button 
            onClick={onClose}
            className={`flex items-center gap-2 transition-colors ${isLight ? 'text-slate-500 hover:text-slate-900' : 'text-gray-400 hover:text-white'}`}
          >
        ...
        <div className="flex items-center gap-4">
          ...
          {availablePages.length > 1 && (
            <div className={`hidden md:flex rounded-lg p-1 ${isLight ? 'bg-slate-100 border border-slate-200' : 'bg-surface-dark border border-panel-border'}`}>
        ...
        {/* Action Buttons */}
        <div className="flex items-center gap-3">
          <div className="flex items-center border-r border-panel-border pr-3 gap-1">
            <button 
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isLight ? 'bg-slate-200 hover:bg-slate-300 text-slate-700' : 'bg-panel-border hover:bg-[#323645] text-white border border-transparent hover:border-gray-600'}`}
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-sm font-bold shadow-lg shadow-primary/25 transition-all glow-primary disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isDeploying ? (
              <>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left Panel: AI Chat or Visual Properties */}
        <aside className={`w-full md:w-[35%] lg:w-[30%] min-w-[320px] max-w-[450px] flex flex-col border-r relative z-10 ${isLight ? 'bg-white border-slate-200' : 'bg-[#111218] border-panel-border'}`}>
          {mode === 'ai' ? (
            <>
