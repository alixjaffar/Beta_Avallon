...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [statsResponse, signupsResponse] = await Promise.all([
        fetch('http://localhost:3000/api/beta-signups?type=stats'),
        fetch('http://localhost:3000/api/beta-signups?type=all'),
      ]);
  ...
  const sendBulkEmail = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/bulk-email', {
        method: 'POST',
        headers: {
  ...
  const deleteSignup = async (signupId: string, signupName: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/beta-signups/${signupId}`, {
        method: 'DELETE',
      });
