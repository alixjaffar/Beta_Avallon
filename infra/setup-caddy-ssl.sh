...
# Configuration
DOMAIN="${N8N_DOMAIN:-n8n.avallon.ca}"
...
read -p "Use domain '$DOMAIN'? (y/n) [y]: " confirm
confirm=${confirm:-y}
if [ "$confirm" != "y" ]; then
...
ufw allow 443/tcp
ufw allow 443/udp
ufw status | grep -E "(80|443)" || echo -e "${YELLOW}Firewall rules may already be configured${NC}"
